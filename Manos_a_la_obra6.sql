...
INSERT INTO creador VALUES(1,'Francisco Naranjo'),('Laureano Cid');
...
SELECT nombre_real from personajes where nombre_real like 'c%';
SELECT inteligencia from personajes order by inteligencia;



SELECT nombre_real FROM personajes WHERE nombre_real like 'b%';

SELECT ocupacion FROM personajes order by ocupacion asc;
/*SELECT nombre/s de la/s columna/s FROM tablaOrigen GROUP BY nombres de
columna/s por la cual Agrupar;*/
SELECT ocupacion,inteligencia from personajes group by ocupacion;

SELECT max(fuerza) as mayor from personajes;
SELECT personaje, MIN(fuerza) AS menor FROM personajes GROUP BY personaje;
SELECT personaje, MIN(fuerza) AS menor FROM personajes GROUP BY personaje HAVING fuerza = MIN(fuerza); 
